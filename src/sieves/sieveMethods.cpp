...
void trialDivision(const long long &lL, const long long &uL,
                   std::vector<long long> &primes,
                   const std::vector<long long> &preSievedPrimes) {
  for (long long i = (lL < 2) ? 2 : lL; i <= uL; ++i) {
    bool isPrime = true;
...
void eratosthenesSieve(const long long &lL, const long long &uL,
                       std::vector<long long> &primes,
                       const std::vector<long long> &preSievedPrimes) {
...
void eulerSieve(const long long &lL, const long long &uL,
                std::vector<long long> &primes,
                const std::vector<long long> &preSievedPrimes) {
  std::vector<bool> isPrime(uL - lL + 1, true);
  for (const long long &p : preSievedPrimes) {
...
void sundaramSieve(const long long &lL, const long long &uL,
                   std::vector<long long> &primes,
                   const std::vector<long long> &preSievedPrimes) {
  const long long &nNew = (uL - 2) / 2;
...
void incrementalSieve(const long long &lL, const long long &uL,
                      std::vector<long long> &primes,
                      const std::vector<long long> &preSievedPrimes) {
  std::vector<bool> isPrime(uL - lL + 1, true);
  for (const long long &p : preSievedPrimes) {
...
void wheelSieve(const long long &lL, const long long &uL,
  ...
  long long start = (lL % 30 == 0) ? 0 : 30 - (lL % 30);
  for (long long i = start; i <= uL - lL; i += 30) {
    for (int j = 0; j < 8; j++) {
      long long index = i + wheel_indices[j];
      if (index <= uL - lL && isPrime[index]) {
        primes.push_back(index + lL);
        for (long long j = (index + lL) * (index + lL); j <= uL;
             j += 2 * (index + lL)) {
          if (j - lL >= 0)
            isPrime[j - lL] = false;
        }

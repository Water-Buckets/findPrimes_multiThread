...
void multiThread(const int &threads, const char &m, const long long &n,
  ...
  if (m == 'e') {
    std::cerr
        << "Error: Trying to use Sieve of Atkin when using multithread mode";
    exit(0);
  }
  ...
  auto diffPre =
      std::chrono::duration_cast<std::chrono::nanoseconds>(endPre - startPre);
  long long durationPre = diffPre.count();
  std::cout << "Time elapsed pre-sieveing:" << double(durationPre) / 1000000000
            << " seconds." << std::endl;
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    if (i == threads - 1)
      uL = n;
    std::thread thr(sievePrimes, std::cref(lL), std::cref(uL), std::cref(m),
  ...
  std::cout << "Time elapsed sieveing:" << double(duration) / 1000000000
            << " seconds." << std::endl;
  ...
  std::clog << "Time elapsed writing to file: "
